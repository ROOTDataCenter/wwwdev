 ...
 */
 ...
 if ( ! class_exists( 'Fusion_Breadcrumbs' ) ) {
	class Fusion_Breadcrumbs {
		...
		private function wrap_breadcrumbs() {
			$this->html_markup = sprintf( '<div class="fusion-breadcrumbs">%s</div>', $this->html_markup );
		}
		...
		private function get_breadcrumb_prefix() {
			...
			if ( ! is_front_page() ) {
				...
				if ( $this->home_prefix ) {
					$prefix = sprintf( '<span class="fusion-breadcrumb-prefix">%s:</span>', $this->home_prefix );
				}
			}
		...
		private function get_post_terms() {
			...
			} elseif ( $this->post->post_type == 'avada_portfolio' ) {
		...
		private function get_single_breadcrumb_markup( $title, $link = '', $separator = TRUE, $microdata = TRUE ) {
			...
			if ( $separator ) {
				$separator_markup = sprintf( '<span class="fusion-breadcrumb-sep">%s</span>', $this->separator );
			}
