	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->strings = array(
				...
				'oops'							=> __( 'Something went wrong.', 'Avada' ),
				'notice_can_install_required'	 => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins installed: %1$s.' ),
				...
				'return'						  => __( 'Return to Plugins Page', 'Avada' ),
		...
		public function init() {
			...
			if ( $this->plugins ) { 
				...
				if ( $this->has_notices ) {
					if( isset( $_GET['page'] ) && $_GET['page'] == 'avada-plugins' ) {
						return;
					}
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) || ( isset( $_GET[sanitize_key( 'tgmpa-update' )] ) && 'update-plugin' == $_GET[sanitize_key( 'tgmpa-update' )] ) ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						if( isset( $_GET['return_url'] ) && $_GET['return_url'] == 'fusion_plugins' ) {
							echo '<p><a href="' . admin_url( 'admin.php?page=avada-plugins' ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . $this->strings['return'] . '</a></p>';
						} else {
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'Avada' ) . '">' . __( 'Return to the Dashboard', 'Avada' ) . '</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						$msg = sprintf( __( 'The following plugin was activated successfully: %s.', 'Avada' ), '<strong>' . $plugin['name'] . '</strong>' );
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) || $update_available ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'Avada' ), $imploded, $count ) . '</p>'; // All messages now stored
					}

					/** Setup variables to determine if action links are needed */
					
					$show_install_link  = $install_link ? '<a class="button-primary" href="' . admin_url( 'admin.php?page=avada-plugins' ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'Avada' ) . '</a>' : '';
					$show_activate_link = $activate_link ? '<a class="button-primary" href="' . admin_url( 'admin.php?page=avada-plugins' ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'Avada' ) . '</a>'  : '';
					$show_update_link  = $update_link ? '<a class="button-primary" href="' . admin_url( 'admin.php?page=avada-plugins' ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'Avada' ) . '</a>' : '';
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice button-secondary" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', 'Avada' ) . '</a>',
						)
					);
					...
					/** Register the nag messages and prepare them to be processed */
			   		if ( isset( $this->strings['nag_type'] ) )
						add_settings_error( 'tgmpa', 'tgmpa', $rendered, 'error' );
					else
						add_settings_error( 'tgmpa', 'tgmpa', $rendered, 'error' );
				}
			}
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', 'Avada' );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', 'Avada' );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', 'Avada' );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', 'Avada' );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', 'Avada' ) : __( 'Recommended', 'Avada' );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'Avada' ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'Avada' ) );
				elseif ( version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Updated', 'Avada' ) );
				else
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Active', 'Avada' ) );
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'Avada' ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'Avada' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'Avada' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'Avada' ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']		  = __( 'Install package not available.', 'Avada' );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'Avada' );
				$this->strings['unpack_package']	  = __( 'Unpacking the package&#8230;', 'Avada' );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'Avada' );
				$this->strings['process_failed']	  = __( 'Plugin install failed.', 'Avada' );
				$this->strings['process_success']	 = __( 'Plugin installed successfully.', 'Avada' );
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					...
					$this->upgrader->strings['skin_update_successful']	= __( '%1$s installed and activated successfully.', 'Avada' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'Avada' ) . '</span><span class="hidden">' . __( 'Hide Details', 'Avada' ) . '</span>.</a>';
				...
				/** Default installation strings */
					...
					$this->upgrader->strings['skin_update_successful']	= __( '%1$s installed successfully.', 'Avada' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'Avada' ) . '</span><span class="hidden">' . __( 'Hide Details', 'Avada' ) . '</span>.</a>';
			...
			public function bulk_footer() {
				...
				foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						if( isset( $_GET['return_url'] ) && $_GET['return_url'] == 'fusion_plugins' ) {
							echo '<p><a href="' . admin_url( 'admin.php?page=avada-plugins' ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . TGM_Plugin_Activation::$instance->strings['return'] . '</a></p>';
						} else {
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'Avada' ) . '">' . __( 'Return to the Dashboard', 'Avada' ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
